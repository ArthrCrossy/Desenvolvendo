...
public class algebraRecursivo {
    ...
    public static char[] transformaChar(String palavra, int i, int size, char conv[]){
        ...
        if(i < size){
			...
			transformaChar(palavra, i, size, conv);
		}
    ...
    public static char[] or(char[] entrada,int j, int stop, int answer, boolean ehtrue, boolean continuar, int i){
			...
			if((entrada[i]=='V') && (!ehtrue)){
                // acha a posição chave
				answer=i;
                // descobre que é verdade
				ehtrue=true;
    ...
    public static void main(String[] args){
        ...
        while( numb!= 0){
            ...
            char[] answer = transformaChar(ex, 0, size, conv);
			...
			printaResposta(0, answer.length, answer);
	...
	// printa respostas
	public static void printaResposta(int i, int size, char[] entrada){
